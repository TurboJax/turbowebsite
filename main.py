...
@app.post("/cnuclasses")
def cnu_api():
    return redirect(f"https://turbojax.org/cnuclasses?semester={request.form["semester"]}")

@app.get("/cnuclasses")
def cnuclasses():
    # Getting semester.  Defaults to the next major one
    semesterList = '202700'
    if 'semester' in request.args:
        semesterList = request.args['semester']
